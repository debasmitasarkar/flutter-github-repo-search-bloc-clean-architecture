...
void main() {
  ...
  setUp(() {
    locator.reset(); // Reset the locator before each test to avoid conflicts
  });

  tearDown(() {
    locator.reset(); // Reset after each test
  });

  test('should register all dependencies', () {
    // Arrange
    setUpLocator(); // Call the function that sets up the locator

    // Act & Assert: Check if dependencies are registered
